...
2. 制定設計策略
3. **提示你切換到 Terminal**
4. **在 Terminal 中暫停並等待你的確認**

⚠️ **重要**：系統會在 Web UI 中提示你，但實際輸入必須在**啟動 `adk web` 的 Terminal 視窗**中進行。
...
#### 為什麼使用 `input()` 而不是 Web UI 確認？

這個設計更貼近實務現場：

**優點**：

- ✅ **簡單直接**：不需要複雜的 Web UI 整合
- ✅ **實務導向**：許多企業內部工具的運作整合

**注意事項**：

- ⚠️ 用戶需要在 Terminal 中輸入（不是 Web UI 的 chat）

**如果你想要 Web UI 整合**：
可以使用 ADK 的 `ToolContext.request_confirmation()` 方法
參考：[ADK Action Confirmations 文檔](https://google.github.io/adk-docs/tools-custom/confirmation/)
